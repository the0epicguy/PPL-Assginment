...
class ToDoApp:
    def __init__(self, root):
        ...
        # Add button (you can insert an image here)
        # add_icon = tk.PhotoImage(file="add_icon.png")
        add_btn = tk.Button(button_frame, text="Add Task", font=("Arial", 12),
                            bg="#4CAF50", fg="white", width=10, relief="flat",
                            cursor="hand2", command=self.add_task)
        add_btn.grid(row=0, column=0, padx=5)

        # Delete button (image can be added)
        # delete_icon = tk.PhotoImage(file="delete_icon.png")
        del_btn = tk.Button(button_frame, text="Delete", font=("Arial", 12),
                            bg="#f44336", fg="white", width=10, relief="flat",
                            cursor="hand2", command=self.delete_task)
        del_btn.grid(row=0, column=1, padx=5)

        # Clear all button (image can be added)
        # clear_icon = tk.PhotoImage(file="clear_icon.png")
        clear_btn = tk.Button(button_frame, text="Clear All", font=("Arial", 12),
                              bg="#9C27B0", fg="white", width=10, relief="flat",
                              cursor="hand2", command=self.clear_tasks)
